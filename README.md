...
Networks
Before defining the networks we have to create some random noise.
...
Starting with the Discriminator that takes flattened images (28x28 = 786) as input and returns the probability if the input might be a real- or a generated fake image. The Discriminators goal is to distinguish whether input images are real or generated by the Generator. 
Each hidden layer is followed by a LeakyRelu and a Dropout layer. The Sigmoid function is used in our output to keep values in the range of (0,1).
